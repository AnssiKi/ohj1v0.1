...
public partial class TeeUusiVaraus : ContentPage
{
    ...
    private async void alue_nimi_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if ((Alue)alue_nimi.SelectedItem != null)
        {
            ...
            using var context = new VnContext();
            {
                ...
                mokitValitullaAlueella = await context.Mokkis.Where(m => m.AlueId == selectedAlue.AlueId).ToListAsync();//filtteroidaan mˆkit alueella
    ...
    {
        //Tarkistetaan viel‰ ettei mˆkill‰ oo samaan aikaan jo olemassa olevia varauksia
        using (var context = new VnContext())
        {
